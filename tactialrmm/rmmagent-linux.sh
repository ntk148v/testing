...
function go_install() {
    if ! command -v go &> /dev/null; then
        case $system in
        amd64) curl -L -k -o /tmp/golang.tar.gz "$go_url_amd64" ;;
        x86) curl -L -k -o /tmp/golang.tar.gz "$go_url_x86" ;;
        arm64) curl -L -k -o /tmp/golang.tar.gz "$go_url_arm64" ;;
        armv6) curl -L -k -o /tmp/golang.tar.gz "$go_url_armv6" ;;
        esac
        rm -rf /usr/local/go/
...
function agent_compile() {
        ...
        #curl -L -k -o /tmp/rmmagent.tar.gz "https://github.com/amidaware/rmmagent/archive/refs/heads/master.tar.gz"
        tar -xf /tmp/rmmagent-master.tar.gz -C /tmp/
...
function install_agent() {
        #cp /tmp/temp_rmmagent /usr/local/bin/rmmagent
        /tmp/temp_rmmagent -insecure -m install -api $rmm_url -client-id $rmm_client_id -site-id $rmm_site_id -agent-type $rmm_agent_type -auth $rmm_auth
        #rm /tmp/temp_rmmagent
...
function install_mesh() {
        curl -L -k -o /tmp/meshagent $mesh_url
